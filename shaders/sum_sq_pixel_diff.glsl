...
void main(){
     ...
     for (int i=0; i<NUM_OFFSETS_SQ; i++) {
          diffs[i] = ( texture(texture_1, vec2(uv_to_frag.x+offsets_2d_81[i].x, uv_to_frag.y+offsets_2d_81[i].y)).r -
                       texture(texture_0, vec2(src_xy.x    +offsets_2d_81[i].x, src_xy.y    +offsets_2d_81[i].y)).r );
     }
     sum = 0.0;
     for (int i=0; i<NUM_OFFSETS_SQ; i++) {
          sum += diffs[i]*diffs[i];
